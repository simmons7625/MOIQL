...
class SSMIQTrainer:
    ...
    def update(self, trajectory: Dict) -> Dict[str, float]:
        ...
        # Update SSM (if Mamba, do trajectory-wise training)
        if len(ssm_losses) > 0:
            mean_ssm_loss = torch.stack(ssm_losses).mean()
            self.ssm.step(mean_ssm_loss)
        ...
        if len(ssm_losses) > 0:
            metrics["ssm_loss"] = mean_ssm_loss.item()

        return metrics

    def save(self, path: str):
        """Save model checkpoint (Q-network and SSM if NeuralSSM)."""
        save_path = Path(path)
